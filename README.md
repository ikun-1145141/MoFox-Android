...
- **Push**：按 `arm64-v8a` / `armeabi-v7a` / `x86_64` 三个 ABI 拉取 jniLibs + rootfs，构建 debug APK 上传 artifact。
- **Nightly**：每天北京时间 02:00 构建三 ABI APK；手动触发可选 `debug` / `release`，定时构建发布到 `nightly` 预发布。
