    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: asyncInstances.when(
          ...
          data: (items) =>
              items.isEmpty ? const _EmptyState() : _InstanceGrid(items: items),
        ),
      ),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            FilledButton.icon(
              onPressed: () =>
                  GoRouter.of(context).go(AppRoute.wizard),
              icon: const Icon(Icons.add),
...
class _InstanceGrid extends StatelessWidget {
  const _InstanceGrid({required this.items});
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final crossAxis = constraints.maxWidth >= 720 ? 2 : 1;
