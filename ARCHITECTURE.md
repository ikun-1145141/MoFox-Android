...
::: info 关于 proot
内置 rootfs 默认通过 **proot 兼容层** 启动 Bot / Napcat。App 先解压 Termux base bootstrap 到应用私有目录，再用内置 `proot` 建立隔离视图（绑定 `$HOME`、`$PREFIX/tmp`、`/proc`、`/dev` 等必要路径），最后在 proot 内执行安装脚本与长期进程。这样启动路径更接近过去 ZeroTermux / proot-distro 的部署习惯，也方便后续切换到 Ubuntu/Debian rootfs。直接 Termux 启动作为性能档保留，只有在用户明确选择时启用。
:::
