...
5. **装运行时依赖**：`pkg install python git`、`pip install uv`，离线包优先 `assets/runtime/wheels/`，缺包再联网
6. **安装 Napcat 并扫码登录**：在 rootfs 里把 Napcat 适配器装上、首次启动；监听 Napcat 输出抓出登录二维码（终端 ANSI 二维码 → 转成图片）直接显示在 OOBE 屏幕中央，下方给一个「打开 QQ 扫一扫」的副按钮辅助。用户扫码 + 手机端确认后，Napcat 把登录态写到 `napcat_adapter/data/`，本步置为完成；超时或用户跳过则允许「稍后在管理页扫码」
7. **拉取 Neo-MoFox**：`git clone` 到 `$HOME/Neo-MoFox`，`uv sync` 同步依赖（流式日志回显到 OOBE 屏幕底部）
8. **生成默认配置**：首次运行后立刻发 SIGINT 优雅关闭，让 Bot 把默认 toml 写出来
9. **填表上线**：表单收集主人 QQ、模型 API Key、Napcat 端口；启用 `[http_router]`、生成随机 `api_keys`，写入 `core.toml` / `model.toml` / `napcat_adapter/config.toml`，并保存到 Secure Storage
...
::: info 关于 proot
内置 rootfs 直接采用 Termux base bootstrap：默认我们 **不再套一层 Ubuntu**，直接用 Termux 自带的 Debian-like 包系统（pkg = apt 的 Termux 封装），少一层 proot 性能更好。Ubuntu 模式作为「兼容档」保留，仅在用户主动开启时通过内置的 `proot` 二进制启动。
:::
...
- **进程托管**：通过原生 `TermuxLauncher`（基于 Android `Process` + JNI fork/exec）以 `filesDir/usr/bin/bash` 为入口拉起 shell，环境变量复用 Termux 标准 `PREFIX/HOME/LD_LIBRARY_PATH/PATH/TMPDIR`。
